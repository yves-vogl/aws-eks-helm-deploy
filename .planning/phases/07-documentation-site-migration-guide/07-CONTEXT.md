...
### D5 — Variable reference generator (Researcher Correction C1 + C2 applied)
- **Locked by:** DOC-02 SC-2.
- **Generator:** `scripts/generate-variables-doc.py` (≤30 LOC wrapper) reads `src/aws_eks_helm_deploy/settings.py` (Pydantic model — `config/` subdir does NOT exist; C1 path fix) and emits `docs/reference/variables.md` via `settings-doc 4.3.2` (MIT, NIH-free; C2 swap).
- **Wrapper responsibility:** invoke `settings-doc generate --class aws_eks_helm_deploy.settings:Settings --output-format markdown` + prepend autogenerated banner.
- **Banner in generated file:** `<!-- AUTOGENERATED FROM src/aws_eks_helm_deploy/settings.py — DO NOT EDIT BY HAND. Regenerate via `uv run scripts/generate-variables-doc.py`. -->`
...
- **Dep added:** `settings-doc == 4.3.2` to `[project.optional-dependencies] docs` (NOT runtime).
...
- **src/aws_eks_helm_deploy/settings.py** — Pydantic Settings model; source of truth for D5 generator (path corrected via Researcher C1).
